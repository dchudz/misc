...
$$\mu_N \sim U[0,1000]$$
$$\sigma_N \sim U[0,1000]$$
$$\sigma_y \sim U[0,\frac{\mu_N}{50}]$$
...
ggplot(data=transform(addZerosToSamples(samples3), N=factor(N))) + geom_line(mapping=aes(x=t, y=theta, color=N, group=paste(N,sample_num))) +
...
set.seed(1)
plot_fake_data(generate_data_including_hyperparameters(50))
...
We can also see if the fits are similar by comparing samples from the posterior for $\{\theta_i\}$ from each model:
...
The posterior distribution for the hyperparameters (especially $\sigma_N$) is very wide. I think there maybe just isn't enough information to fit them well, so (unfortunately) the prior has a large influence. I haven't figured out a good way to handle that in this case. 

```{r posterior_hyperparameters, dependson="fit_model",error=FALSE, warning=FALSE, results='hide', message=FALSE, echo=FALSE}
mu_N_posterior <- plot_posteriors(fits1, "mu_N")
sigma_N_posterior <- plot_posteriors(fits1, "sigma_N")
#plot_posteriors_both_models(fits1, "sigma_y")

sigma_N_fit <- extract(fits1$fit, pars="sigma_N")[["sigma_N"]]
mu_N_fit <- extract(fits1$fit, pars="mu_N")[["mu_N"]]
...
truth
true_ratio <- as.numeric(truth[2]**2/truth[1])
s <- extract(fit, pars=par)
posterior_ratio <- sapply(1:length(s$sigma_N), function(i) s$sigma_N[i]**2/s$mu_N[i])
