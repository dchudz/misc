...
(I should change mu's to theta's here and in the code...)

*This post will describe a way I came up with of doing Bayesian isotonic regression using [Stan](http://mc-stan.org/). If you want practical help, expert research, or to learn anything about what's standard in the field, this isn't the place for you (Andrew Gelman pointed me to ___'s work which seems a bit different, so maybe check him out) . This **is** the place for you if you want to read about how I thought about setting up a model, implemented the model in Stan, and created graphics to understand what was going on.*
...
So what model should we use instead? For my convenience (and perhaps to be more realistic) I'll switch to the case where $mu_N$ ($f(1)$) isn't known to be $1$. The goal is the have a model which will behave in the same way at each scale of discretization -- our results shouldn't differ too much when $N=10$ vs. $N=100$. To accomplish this, I model $mu_N$ as a jump drawn from a chi squared distribution. This is nice since 1 jump that's distributed as $chi-squared(k)$  is then equivalent to  $n$  independent jumps that are distributed as $chi-squared(k/n)$. Then our prior for the function $f$  is independent of the scale of the discretization, as long as we scale the parameter correctly:

* $$mu_i ~ mu_{i-1} + s*chi_square(k_0/N)$$ (the factor of $s$ lets us control the mean/variance of the jumps independently)

I chose to express $s$ and $k_0$ in terms of the mean and variance of $mu_N$:

$$s = ...\mu_N ... \sigma_N ...$$
$$k_0 = ...\mu_N ... \sigma_N ...$$


I'll assume that we have data $y_i$ from a normal distribution centered on $mu_i$. The variance of the $y$'s  should scale with 1/N, since I'm assuming that the $y$'s at a coarser scale should be thought of as an average of the $y$'s at a finer scale:

y_i ~ N(mu_i, sqrt({sigma_y}^2*N))

We need priors for $\mu_N$, $\sigma_N$, and $sigma_y$. This would naturally depend on the problem, but I decided that both the variance of $\theta_N$ and the variance of the data $y_i$ should be sensitive to the mean $\mu_i$ of $\theta_n$:

$$\mu_N ~ $$
$$\sigma_N ~ $$
$$\sigma_y ~ $$

Here are some samples from my prior for the $\mu_i$:
