...
The posterior distribution of the hyperparameters is also fairly similar between the two models, but sharper for the unaggregated version (as I think you'd expect). I may have made a bad choice parametrizing the model, since there seems to be a lot more trouble estimating $\sigma_N$ and $\mu_N$ than their ratio (which is ) 

```{r posterior_hyperparameters, dependson="fit_model",error=FALSE, warning=FALSE, results='hide', message=FALSE, echo=FALSE}
mu_N_posterior <- plot_posteriors_both_models(fits1, "mu_N")
sigma_N_posterior <- plot_posteriors_both_models(fits1, "sigma_N")
